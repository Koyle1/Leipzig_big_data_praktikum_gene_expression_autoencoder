...
class CellVAE(nn.Module):
    ...
    def loss_function(self, recon_x, x, mu, logvar, values=None, sparsity_logits=None,
        ...
        # Method 1: Weighted Binary Cross-Entropy
        # Calculate positive weight to heavily penalize false negatives
        num_zeros = (is_nonzero == 0).sum()
        num_nonzeros = (is_nonzero == 1).sum()
        
        if num_nonzeros > 0:
            # Weight false negatives much more heavily than false positives
        ...
        # Method 2: Additional explicit false negative penalty
        sparsity_probs = torch.sigmoid(sparsity_logits)
        pred_nonzero = (sparsity_probs > 0.5).float()
        
        # False negatives: predicted zero but actually non-zero
        false_negatives = ((pred_nonzero == 0) & (is_nonzero == 1)).float()
        fn_penalty_loss = false_negatives.mean() * fn_penalty
        
        # Method 3: Recall-focused loss component
        # Penalize low recall (missing true positives)
        ...
        # RMSE for value prediction (only on non-zero elements)
        ...
        return total_loss, value_rmse, total_sparsity_loss, KLD, sparsity_accuracy
